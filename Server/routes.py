...
def register():
    ...
    if validate_username(username) and validate_email(email) and validate_password(password): 
        ...
        db.session.add(user)
        db.session.commit()
        return jsonify([True, ProtocolErrors.PARAMETERS_DOES_NOT_MATCH_REQUIREMENTS.value])

    return jsonify([False, ProtocolErrors.PARAMETERS_DOES_NOT_MATCH_REQUIREMENTS.value]) #add custom error msg later
