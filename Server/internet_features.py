...
class InternetFeaturesHandler:
	...
	def wiki_search(self, keyword):
		...
		if len(search_results) == 0:
			raise InternetFeatures.NoResultsFound

		for result in search_results: #getting the first result which is a real page 
			try:
				return wikipedia.summary(result, sentences=self._sentence_count , auto_suggest=False)
